...
<section id="home" class="hero-wrap text-white">
    ...
    <div class="max-w-6xl mx-auto px-6 pb-12">
        <div class="flex items-center justify-between text-white/80 text-sm mb-4">
            <span>Featured studio builds</span>
            <div class="h-px flex-1 ml-4 bg-white/20"></div>
        </div>
        <div id="studio-slider" class="tiny-slider">
            <?php foreach ($sliderItems as $item): ?>
                <article class="slider-card">
                    <div>
                        <p class="text-white/60 text-xs tracking-[0.4em] uppercase mb-2">0<?= $loopIndex ?? '' ?></p>
                        <h4><?= h($item['title']) ?></h4>
                    </div>
                    <p><?= h($item['copy']) ?></p>
                </article>
...
<section id="about" class="bg-white py-16">
    <div class="max-w-6xl mx-auto px-6 grid gap-8 md:grid-cols-2 items-start">
        <div class="space-y-5">
            <p class="text-sm uppercase tracking-[0.3em] text-smdark/60">About School Media</p>
...
<section id="programs" class="bg-smlight py-16">
    <div class="max-w-6xl mx-auto px-6 text-center mb-10">
        <p class="text-sm uppercase tracking-[0.3em] text-smdark/60">Signature tracks</p>
...
<section id="contact" class="bg-white py-16">
    <div class="max-w-6xl mx-auto px-6 text-center mb-10">
        <p class="text-sm uppercase tracking-[0.3em] text-smdark/60">Contact us</p>
    ...
    <div class="max-w-6xl mx-auto px-6 grid gap-8 md:grid-cols-2">
        ...
        <div class="program-card rounded-3xl bg-white p-6">
            <form class="space-y-4">
                <input class="w-full rounded-2xl border-slate-200 focus:border-smsecondary focus:ring-smsecondary" type="text" placeholder="Full Name" required>
                <input class="w-full rounded-2xl border-slate-200 focus:border-smsecondary focus:ring-smsecondary" type="email" placeholder="Work Email" required>
                <input class="w-full rounded-2xl border-slate-200 focus:border-smsecondary focus:ring-smsecondary" type="text" placeholder="School or District">
                <textarea class="w-full rounded-2xl border-slate-200 focus:border-smsecondary focus:ring-smsecondary" rows="4" placeholder="How can we help?"></textarea>
...
<section id="map" class="bg-smlight py-16">
    <div class="max-w-6xl mx-auto px-6 text-center mb-8">
        <p class="text-sm uppercase tracking-[0.3em] text-smdark/60">Location</p>
